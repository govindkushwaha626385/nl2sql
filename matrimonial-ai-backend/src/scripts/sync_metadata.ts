...
export async function syncMetadata() {
  ...
  for (const row of tables) {
    ...
    const columnList = columns.map((c: { column_name: string; data_type: string }) => `${c.column_name} (${c.data_type})`).join(', ');
...
function inferModule(tableName: string): string {
  if (tableName.includes('horoscope') || tableName.includes('astro')) return 'Astrology';
  if (tableName.includes('payment') || tableName.includes('sub')) return 'Finance';
  if (tableName.includes('profile') || tableName.includes('user')) return 'Core';
  return 'General';
}
