...
export async function getRelevantContext(userQuestion: string) {
  const queryEmbedding = await getEmbedding(userQuestion);
  const vectorStr = `[${queryEmbedding.join(',')}]`;

  const results = await db.raw(
    `SELECT table_name, description, module_name
  ...
  const rows = (results as { rows: { table_name: string; description: string; module_name: string }[] })
    .rows;
